...
DISPATCH_EVIDENCE_DB_URL="${AURIS_E2E_DISPATCH_EVIDENCE_DATABASE_URL:-${DB_URL:-${DATABASE_URL:-}}}"
if [ -z "${DISPATCH_EVIDENCE_DB_URL}" ]; then
  echo "UI/BFF E2E requires DATABASE_URL or AURIS_E2E_DISPATCH_EVIDENCE_DATABASE_URL for scoped read-only dispatch evidence." >&2
  exit 2
fi

env AURIS_E2E_URL="${E2E_URL}" \
  AURIS_E2E_BFF_URL="${BFF_URL:-${E2E_URL}}" \
  DATABASE_URL="${DISPATCH_EVIDENCE_DB_URL}" \
  AURIS_E2E_DISPATCH_EVIDENCE_PYTHON="${PYTHON_BIN}" \
  AURIS_E2E_DISPATCH_EVIDENCE_HELPER="${ROOT}/scripts/process_e2e_outbox_run.py" \
