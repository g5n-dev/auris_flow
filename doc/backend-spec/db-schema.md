...
| `task_runs` | 任务运行主表 | `task_run_id`、`tenant_id`、`project_id`、`task_version_id`、`trigger_type`、`partition_key`、`status`、`dagster_run_id`、`idempotency_key`、`started_at`、`finished_at`、`retry_count`、`error_code`、`trace_id` | `uk_task_run_idempotency(tenant_id,project_id,idempotency_key)`；`idx_task_runs_version(task_version_id,status,created_at)`；`idx_task_runs_partition(tenant_id,project_id,partition_key,status)` | 统一运行状态；不入 Qdrant。 |
...
| `label_recomputation_runs` / `label_recomputation_run_items` | split/语义变化重算 | scope、target/bundle、source Head generations、partitions、fact cutoff、budget、attempt/status、result refs/hashes、Trace | scope run/idempotency 与 run/partition/attempt 唯一 | candidate namespace 在批准晋级前不可用于生产读；部分失败不可晋级。 |
...
| `release_bundle_head_events` | 环境 activation append-only ledger | scope、environment、generation、old/new deployment/bundle/label version、active/draining/inactive/rolled-back interval、command/receipt、event SHA、Trace | scope/environment/generation 唯一；复合 FK；UPDATE/DELETE trigger | `release_bundle_heads` 仅当前投影；运行受理冻结 Head generation，制品状态不能替代环境状态。 |
