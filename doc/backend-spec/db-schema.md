...
| `oidc_authorization_states` | OIDC Code + PKCE 一次性浏览器握手 | `state_sha256`、`transaction_sha256`、`nonce`、`code_verifier`、`return_path`、`issued_at`、`expires_at`、`consumed_at`、timestamps | state hash 主键；transaction hash 唯一；transaction/consumed/expiry 与 expiry/consumed 索引；时间 CHECK | state 与短期 HttpOnly transaction cookie 原值均不落库；回调恒定时间核对浏览器绑定后才原子消费，所有终态清除 cookie；同一浏览器最多一个待处理握手，过期/已消费记录按有界批次清理；return path 仅允许站内路径。 |
| `browser_auth_sessions` | BFF 不透明 HttpOnly 浏览器会话 | `browser_session_id`、`token_sha256`、`csrf_sha256`、`oidc_identity_id`、`user_id`、`tenant_id`、`project_id`、`oidc_session_id_sha256`、`provider`、`issued_at`、`expires_at`、`revoked_at`、`last_seen_at`、timestamps | token SHA 唯一；identity/user/tenant/project FK；user/revoked/expiry、identity/expiry 与 OIDC sid hash/revoked/expiry 索引；时间 CHECK | cookie、CSRF 与 IdP sid 原值永不落库/日志；每次请求回查当前主体状态和项目角色；本地或经验证的 Back-Channel Logout 原子撤销。 |
| `oidc_logout_token_replays` | OIDC Back-Channel Logout 持久化 replay tombstone | `logout_event_sha256`、`issuer_sha256`、`jti_sha256`、`issued_at`、`expires_at`、timestamps | `(issuer_sha256,jti_sha256)` 唯一；expiry 索引；时间 CHECK | 不保存 Logout Token、issuer 或 jti 原值；唯一约束承担跨进程原子防重放，与会话撤销同事务；每次有效请求机会式删除至多 256 个过期 tombstone，接收后至少保留 10 分钟。 |
